...
perl $0 pep.fasta split_num(1000) cpu(8) mem(25g) path(pwd) 
...
for (1..$mark) { 
	print COMMAND "/export/databases/third_part/interproscan-5.50-84.0/interproscan.sh -i $path/$out_name.tmp/$out_name.$_.fasta   -d $path//$out_name.out  -goterms -iprlookup -pa -dp -T $path/$out_name \n";}
...
while (<JOB>){
	chomp;
	open SUBMIT, ">IN$out_name.$job_mark.sh";
	print SUBMIT "$_\n";
	system `qsub  -pe smp $cpu -l vf=$mem IN$out_name.$job_mark.sh`;
	$job_mark++;
	close SUBMIT;
}
